...
public class AndroidAIMLProcessor {
    ...
    public static ArrayList<Category> AIMLToCategories(String directory, String aimlFile) {
        ...
        try {
            ...
            XmlPullParser xml;      // <aiml> tag
            if(!Ghost.isInternalStorage()) {
                xml = AndroidDomUtils.parseFile(directory + "/" + aimlFile);
            }
            else{
                xml = AndroidDomUtils.parseFile(new FileInputStream(new File(Ghost.getFilesDir(),directory + "/" + aimlFile)));
            }
