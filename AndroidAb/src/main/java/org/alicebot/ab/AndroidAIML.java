...
public class AndroidAIML {
    ...
    public String respond(String input, String that, String topic, Chat chatSession, int srCnt) {
        ...
        try {
            ...
            filenames.add(leaf.category.getFilename());
            inputs.add(leaf.category.getPattern());
            Log.d(TAG,"Filename : " + leaf.category.getFilename() );
     ...
     */
    private String unevaluatedXML(String result, Node node, ParseState ps) {
    ...
    private String id(Node node, ParseState ps) {
     ...
     */
    private String size(Node node, ParseState ps) {
     ...
     */
    private String vocabulary(Node node, ParseState ps) {
     ...
     */
    private String program(Node node, ParseState ps) {
     ...
     */
    public boolean evalTagForLoop(Node node) {
        NodeList childList = node.getChildNodes();
        for (int i = 0; i < childList.getLength(); i++)
     ...
     * @return              true or false.
     */
    public boolean validTemplate(String template) {
        try {
            template = "<template>"+template+"</template>";
